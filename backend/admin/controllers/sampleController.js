...
const createSample = async (req, res) => {
  try {
    ...
    // Admin-created samples are auto-approved
...
// @desc    Approve or reject sample
// @route   PUT /api/admin/samples/:id/approve
// @access  Private/Admin
const approveSample = async (req, res) => {
  try {
    ...
    if (!approvalStatus || !["Approved", "Rejected"].includes(approvalStatus)) {
      return res.status(400).json({
        success: false,
        message: "Please provide valid approvalStatus (Approved or Rejected)",
      });
    }
    ...
    res.status(200).json({
      success: true,
      message: `Sample ${approvalStatus.toLowerCase()} successfully`,
      data: populatedSample,
    });
  } catch (error) {
    res.status(500).json({
      success: false,
      message: error.message || "Error updating sample approval",
    });
  }
};
...
const getSampleStats = async (req, res) => {
  try {
    ...
    const convertedSamples = await Sample.countDocuments({
      status: "Converted",
    });
