...
const getAssociationTypeId = async (fromType, toType) => {
  ...
  try {
    ...
    const res = await axios.get(`${HUBSPOT_API_BASE}/crm/v4/associations/${from}/${to}/labels`, {
      headers,
    });
    ...
    console.error(`Error getting association labels for ${fromType}->${toType}:`, e.response?.data || e.message);
...
const associateObjects = async (fromType, fromId, toType, toId) => {
  ...
  if (!typeId) {
    console.warn(`No associationTypeId found for ${fromType}->${toType}. Skipping association.`);
    return false;
  }
  ...
  try {
    ...
    console.error(`Error associating ${fromType}:${fromId} -> ${toType}:${toId}:`, e.response?.data || e.message);
    return false;
  }
};
...
const getObjectProperties = async (objectType) => {
  if (objectPropertiesCache.has(objectType)) return objectPropertiesCache.get(objectType);
  const headers = await getHeaders();
  if (!headers) return [];
  try {
    const res = await axios.get(`${HUBSPOT_API_BASE}/crm/v3/properties/${objectType}`, { headers });
    const props = res.data?.results || [];
    objectPropertiesCache.set(objectType, props);
    return props;
  } catch (e) {
    console.error(`Error fetching HubSpot properties for ${objectType}:`, e.response?.data || e.message);
...
const buildRequiredPropertiesPayload = async (objectType, provided = {}) => {
  ...
  const requiredProps = props.filter((p) => p?.required && !p?.readOnly && p?.name);
...
const getHeaders = async () => {
  ...
  if (config.HUBSPOT_AUTH_MODE === 'oauth') {
    ...
    accessToken = normalizeToken(
      config.HUBSPOT_TOKEN || config.HUBSPOT_ACCESS_TOKEN || config.HUBSPOT_API_KEY
    );
  }
...
const createOrUpdateContact = async (customerData) => {
  try {
    ...
    const contactProperties = stripEmptyValues({
      ...(isValidEmail(customerData.email) ? { email: String(customerData.email).trim() } : {}),
      firstname: firstName,
    ...
    const safeContactProperties = await filterToKnownProperties('contacts', contactProperties);
    ...
    if (isValidEmail(customerData.email)) {
      try {
        ...
        console.log('Contact not found, creating new:', error.response?.data?.message || error.message);
      }
    }
    ...
    console.error('Error syncing contact to HubSpot:', error.response?.data || error.message);
    return null;
  }
};
...
const createDeal = async (quotationData, contactId = null) => {
  try {
    ...
    const dealProperties = await buildRequiredPropertiesPayload('deals', provided);
    ...
    if (contactId && dealId) {
      try {
        const ok = await associateObjects('deals', dealId, 'contacts', contactId);
    ...
    if (dealId && items.length > 0) {
      for (const item of items) {
        try {
          ...
          const lineItemProps = await buildRequiredPropertiesPayload('line_items', lineItemProvided);
          ...
          if (lineItemId) {
            const ok = await associateObjects('deals', dealId, 'line_items', lineItemId);
    ...
    console.error('Error creating deal in HubSpot:', error.response?.data || error.message);
    return null;
  }
};
 ...
 */
const createTimelineEvent = async (contactId, eventType, subject, body, metadata = {}) => {
  try {
    const headers = await getHeaders();
    ...
    console.error('Error creating timeline event in HubSpot:', error.response?.data || error.message);
    return null;
  }
};
...
const createNote = async (contactId, note, noteType = 'GENERAL') => {
  try {
    ...
    console.error('Error creating note in HubSpot:', error.response?.data || error.message);
    return null;
  }
};
...
const updateContactProperty = async (contactId, property, value) => {
  try {
    ...
    console.error('Error updating contact property in HubSpot:', error.response?.data || error.message);
    return false;
  }
};
...
const findContactByEmail = async (email) => {
  try {
    ...
    console.error('Error searching contact in HubSpot:', error.response?.data || error.message);
    return null;
  }
};
 ...
 */
const createTask = async (contactId, taskTitle, taskBody, dueDate = null, taskType = 'TODO') => {
  try {
    const headers = await getHeaders();
    if (!headers || !contactId) return null;
    ...
    const taskData = {
      ...
      metadata: {
        ...
        subject: taskTitle,
        taskType: taskType,
        ...(dueDate && { forObjectType: 'CONTACT', timestamp: new Date(dueDate).getTime() }),
      },
    };
...
const createCustomerInHubSpot = async (customer) => {
  try {
    ...
    console.error('Error creating customer in HubSpot:', error.response?.data || error.message);
    return null;
  }
};
...
const createOrderInHubSpot = async (order, customerId) => {
  try {
    ...
    const orderProperties = await buildRequiredPropertiesPayload('orders', provided);

    console.log('[HUBSPOT] Creating Orders object with properties keys:', Object.keys(orderProperties));
    const response = await axios.post(
    ...
    if (customerId && orderId) {
      try {
        const okOrder = await associateObjects('orders', orderId, 'contacts', customerId);
    ...
    console.error('Error creating order in HubSpot:', error.response?.data || error.message);
    return null;
  }
};
...
const fetchCustomers = async () => {
  try {
    ...
    console.log('Token present:', !!(config.HUBSPOT_API_KEY || config.HUBSPOT_ACCESS_TOKEN));
    
    // HubSpot API expects properties as comma-separated string in query params
    const properties = ['firstname', 'lastname', 'email', 'phone', 'company', 'address', 'city', 'state', 'zip'].join(',');
    ...
    console.log('HubSpot API Response Data Keys:', Object.keys(response.data || {}));
    
    const customers = response.data?.results || [];
    console.log(`Successfully fetched ${customers.length} customers from HubSpot`);
    
    // Log full response structure for debugging
    if (customers.length === 0) {
      console.log('No customers found. Response structure:', JSON.stringify(response.data, null, 2));
    } else {
...
const fetchOrders = async () => {
  try {
    ...
    try {
      ...
      const response = await axios.get(`${HUBSPOT_API_BASE}/crm/v3/objects/orders`, {
        headers,
      ...
      console.log(`Successfully fetched ${orders.length} orders from HubSpot (Orders API)`);
      if (orders.length > 0) console.log('Sample order:', JSON.stringify(orders[0], null, 2));
      return orders;
    } catch (orderError) {
      console.log('Orders API failed, trying Deals API...');
      console.log('Orders API Error:', orderError.response?.status, orderError.response?.data?.message || orderError.message);
    }

    // Fallback to Deals API
    const dealProperties = ['dealname', 'amount', 'dealstage', 'pipeline', 'closedate', 'description'].join(',');
    ...
    console.log(`Successfully fetched ${deals.length} deals from HubSpot (Deals API fallback)`);
    if (deals.length > 0) console.log('Sample deal:', JSON.stringify(deals[0], null, 2));
    return deals;
  } catch (error) {
...
const fetchTasks = async () => {
  try {
    ...
    const response = await axios.get(`${HUBSPOT_API_BASE}/crm/v3/objects/tasks`, {
      headers,
      params: {
        limit: 100,
...
const createTaskObjectInHubSpot = async (task = {}) => {
  try {
    ...
    const hs_timestamp = due && !Number.isNaN(due.getTime()) ? String(due.getTime()) : undefined;
    ...
    console.error('Error creating task object in HubSpot:', error.response?.data || error.message);
    return null;
  }
};
...
const syncHubSpotData = async () => {
  try {
    ...
    console.log(`HubSpot sync: ${customers.length} customers, ${orders.length} orders fetched`);
