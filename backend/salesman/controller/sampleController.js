...
const getSamples = async (req, res) => {
  try {
    ...
    // Show all samples assigned to this salesman (admin-created Sample Track + own; no approval filter so all show)
    const baseFilter = { salesman: req.user._id };
...
const createSample = async (req, res) => {
  try {
    ...
    // Verify customer exists and is related to this salesman (via Customer Allotment or task/visit) if provided
    if (customer) {
      const customerId = (customer && typeof customer === 'object' && customer._id) ? customer._id : customer;
      ...
      // Same logic as getMyCustomers: allotted to me OR has task/visit with me (direct DB queries to avoid type/read issues)
      const allottedToMe = await Customer.findOne({
        _id: customerId,
        allottedSalesman: req.user._id,
      })
        .select('_id')
        .lean()
        .then(Boolean);
      const hasTaskOrVisit =
        (await FollowUp.findOne({ salesman: req.user._id, customer: customerId }).select('_id').lean().then(Boolean)) ||
        (await VisitTarget.findOne({ salesman: req.user._id, customerId }).select('_id').lean().then(Boolean));
      if (!allottedToMe && !hasTaskOrVisit) {
        return res.status(400).json({
          success: false,
          message: 'Customer not assigned to you. Ask admin to allot this customer to you from Customer Allotment, or use a customer that has a task/visit with you.',
        });
      }
    }
    ...
    // Salesman-created samples: no admin approval needed
    const customerIdForCreate = (customer && typeof customer === 'object' && customer._id) ? customer._id : customer;
    const sample = await Sample.create({
      salesman: req.user._id,
      customer: customerIdForCreate || undefined,
      customerName,
      ...
      status: 'Pending',
      approvalStatus: 'Approved', // Salesman-created samples: no admin approval needed
